...
- Prompt: Move 'Request Money' button inside the Top Up dropdown menu.
- Changes:
  - components/Navbar/Navbar.tsx: Moved "Request Money" from a standalone button to the first item in the "Top Up" dropdown menu. Added a divider between it and the top-up options.
- Notes: none

## 2026-02-15

- Prompt: Fix audit report - restore broken auth, retry logic, AbortSignal, CSRF backend, inline styles.
- Changes:
  - auth.ts: Restored localStorage token storage, setToken, clearSession, loadActor passes token to apiRequest.
  - client.ts: Restored X-Token header; token from options or useAuthStore.getState().token.
  - main.tsx: Retry logic now checks ApiError instead of Response for 4xx skip.
  - ProfileStatistics.tsx: Pass signal to getEntityStatsBundle in queryFn.
  - App.tsx: PageLoader uses Tailwind classes instead of inline style.
  - Profile.tsx: CopyErrorAlert uses Mantine Text instead of inline style.
  - api: CSRF middleware (csrf.py), config.csrf_disabled, conftest sets REFINANCE_CSRF_DISABLED; fixed conftest f-string syntax.
- Notes: Auth was broken by prior cookie-based change; backend never supported cookies. CSRF excluded for /tokens, /deposit-callbacks.
