...
export type StatsFilterPreset = 'last4w' | 'last3m' | 'last6m' | 'ytd' | 'custom';

export type StatsFiltersValue = {
  from: string;
  to: string;
  grain: StatsGrain;
...
export const DEFAULT_STATS_GRAIN: StatsGrain = 'month';
export const DEFAULT_STATS_PRESET: Exclude<StatsFilterPreset, 'custom'> = 'last3m';

export const STATS_PRESET_OPTIONS: Array<{
  key: Exclude<StatsFilterPreset, 'custom'>;
  label: string;
}> = [
...
export const isStatsFilterPreset = (value: string | null): value is StatsFilterPreset =>
  value === 'last4w' ||
  value === 'last3m' ||
  value === 'last6m' ||
  value === 'ytd' ||
...
export const areStatsFiltersEqual = (
  left: StatsFiltersValue,
  right: StatsFiltersValue
): boolean =>
