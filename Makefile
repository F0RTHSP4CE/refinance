...
.PHONY: migrate migration migrate-down migrate-history

migrate:
	# Apply all pending migrations. Override env: make ENV=prod migrate
	$(COMPOSE) exec api alembic upgrade head

migration: ENV = dev
migration:
	# Generate a new autogenerated revision. Example: make migration NAME=add_user_email
	@if [ -z "$(NAME)" ]; then echo "Usage: make migration NAME=<short_desc>"; exit 1; fi
	$(COMPOSE) exec api alembic revision --autogenerate -m "$(NAME)"

migrate-down:
	# Roll back the most recent migration. Override env: make ENV=prod migrate-down
